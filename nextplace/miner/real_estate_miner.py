...
from nextplace.miner.watchdog.watchdog import Watchdog
...
class RealEstateMiner(BaseMinerNeuron):
    ...
    def run_watchdog(self):
        """
        RUN IN THREAD
        Every minute check if a synapse has come in the last 5 minutes. If not, restart process
    ...
    def forward(self, synapse: RealEstateSynapse) -> RealEstateSynapse:
        self.database_manager.update_synapse_timestamp()  # Update the synapse table with current timestamp
